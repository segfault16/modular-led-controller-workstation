...
class Test_Panelize(unittest.TestCase):
    ...
    def test_makeSquare8x8_works(self):
     ...
     #    self._makeSquare_works()

    def _makeSquare_works(self, square_size, num_cols):
        self.assertEqual(square_size%num_cols, 0)
        num_rows = int(square_size/num_cols)
        effect = panelize.MakeSquare()
        effect.setNumOutputPixels(square_size)
        effect.setNumOutputCols(num_cols)
        input = np.array([[i+1,2*i+1,3*i+1] for i in range(0,num_rows)]).T
        ...
        self.assertEqual(np.size(output, axis=1), square_size)
        for i in range(0, num_rows):
